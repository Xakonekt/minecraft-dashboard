...
export function getContainer() {
  return docker.getContainer(config.docker.containerName);
}
...
export async function restartContainer() {
  await getContainer().restart();
}

export async function getContainerLogs(tail = 200) {
  const container = getContainer();
  const info = await container.inspect();
...
export async function streamContainerLogs(onLog) {
  const container = getContainer();
  const info = await container.inspect();
  const isTTY = info.Config.Tty;
...
function extractMuxedText(buffer) {
  ...
  // Check if it looks like a multiplexed header (first byte 0 or 1 or 2)
  const streamType = buffer[0];
  if (streamType <= 2 && buffer.length >= 8) {
    const size = buffer.readUInt32BE(4);
    if (size + 8 === buffer.length) {
      return buffer.slice(8).toString('utf8');
    }
  }
